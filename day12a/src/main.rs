...
impl Graph {
    ...
    fn traverse(&mut self, from: usize, mut visited: HashSet<usize>) -> i32 {
        visited.insert(from);
        ...
        for to in 0..paths.len() {
            ...
            if is_valid_path == 1 || is_valid_path == -1 {
                if self.get_cave("start") == to {
                    ...
                    count += self.traverse(to, visited.clone());
                }
            }
        }
...
fn main() {
    ...
    let result = graph.traverse(graph.get_cave("start"), HashSet::new());
